            ...
            (let* ((addiditonal-uniforms (append vert-uniforms frag-uniforms geom-uniforms tcs-uniforms tes-uniforms))
              ...
              (if (not (compile-and-link-shader shader))
                  (begin
              	  ...
              	  (show-errors name vert-source (vertex-shader-info-log shader) "vertex info log")       ;  (format #t "vertex info log:~%~a~%---------~%" (vertex-shader-info-log shader))
              	  (show-errors name frag-source (fragment-shader-info-log shader) "fragment info log")   ;  (format #t "fragment info log:~%~a~%---------~%" (fragment-shader-info-log shader))
              	  (if geometry-shader (show-errors name geom-source (geometry-shader-info-log shader) "geometry info log"))  ;  (format #t "geometry info log:~%~a~%---------~%" (geometry-shader-info-log shader)))
